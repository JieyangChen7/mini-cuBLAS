...
for (int T = 16; T <= min(1024, m); T *= 2) {
   // int T = 128;
    ...
    long long total_bytes = (m * k + k * 4 * (k / 32)) * sizeof(float);
...
float test_kernel_shared(int m, int n, int k, 
    ...
    for (int T = 16; T <= min(1024, m); T *= 2) {
      ...
      cudaEventRecord(start);
      for (int i = 0; i < TEST_RUN; i++)
        dgemm_kernel_shared<<<blocksPerGrid, threadsPerBlock,  T * sizeof(float) * 4>>>(m, n, k, T, dA, lda, dB, ldb, dC, ldc);
        check_cuda_error();
      cudaEventRecord(stop);
...
dgemm_kernel4_2(int m, int n, int k, int T, int t, float * A, int lda, float * B, int ldb, float * C, int ldc)
{
  ...
  __syncthreads();
  cacheB[threadIdx.x * 4] = b0;
  cacheB[threadIdx.x * 4 + 1] = b1;
  cacheB[threadIdx.x * 4 + 2] = b2;
  cacheB[threadIdx.x * 4 + 3] = b3;
  __syncthreads();
...
__global__ void
dgemm_kernel4_23(int m, int n, int k, int T, int t, float * A, int lda, float * B, int ldb, float * C, int ldc)
{
  // store B (T * 2)                                                                                                                                                                                                                                                                       
  extern __shared__ float cacheB[];
...
__global__ void
dgemm_kernel4_22(int m, int n, int k, int T, int t, float * A, int lda, float * B, int ldb, float * C, int ldc)
{
  // store B (T * 2)                                                                                                                                                                                                                                                                       
  extern __shared__ float cacheB[];
...
float test_kernel_prefetch3(int m, int n, int k, 
    ...
    int tt = 4;
      ...
      cudaEventRecord(start);
      for (int i = 0; i < TEST_RUN; i++) {
        dgemm_kernel4_2<<<blocksPerGrid, threadsPerBlock, (T * 4) * sizeof(float)>>>(m, n, k, T, tt, dA, lda, dB, ldb, dC, ldc);
        //dgemm_kernel4_22<<<blocksPerGrid, threadsPerBlock, (T * 2) * sizeof(float)>>>(m, n, k, T, tt, dA, lda, dB, ldb, dC, ldc);
      ...
      float real_time = milliseconds / 1000;
      long long total_bytes = (m * k+ k * 4 * T) * sizeof(float) ;
      float total_gb = (float)total_bytes / 1e9;
      total_gb *= TEST_RUN;
