...
dgemm_kernel4_2_iter(int m, int n, int k, int T, int t, double * A, int lda, double * B, int ldb, double * C, int ldc)
{
  ...
  for (int j = 0; j < k; j += T){ 
    ...
    for (int l = j; l < j + T; l += t){
      ...
      for (int s = 0; s < 4; s ++) {
          temp1 += cr0 * cacheB[l - j + s ];
          temp2 += cr0 * cacheB[l - j + s + 1];
          temp3 += cr0 * cacheB[l - j + s + 2];
          temp4 += cr0 * cacheB[l - j + s + 3];
          temp5 += cr0 * cacheB[l - j + s + 4];
          temp6 += cr0 * cacheB[l - j + s + 5];
          temp7 += cr0 * cacheB[l - j + s + 6];
          temp8 += cr0 * cacheB[l - j + s + 7];
          temp9 += cr0 * cacheB[l - j + s  + 8];
          temp10 += cr0 * cacheB[l - j + s + 9];
          temp11 += cr0 * cacheB[l - j + s + 10];
          temp12 += cr0 * cacheB[l - j + s + 11];
          temp13 += cr0 * cacheB[l - j + s + 12];
          temp14 += cr0 * cacheB[l - j + s + 13];
          temp15 += cr0 * cacheB[l - j + s + 14];
          temp16 += cr0 * cacheB[l - j + s + 15];
      }
      // temp1 += cr0 * cacheB[l - j + 0 ];
      // temp2 += cr0 * cacheB[l - j + 0 + 1];
      // temp3 += cr0 * cacheB[l - j + 0 + 2];
